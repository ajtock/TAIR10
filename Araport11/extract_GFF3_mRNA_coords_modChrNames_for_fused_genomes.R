...
# Usage:
# ./extract_GFF3_mRNA_coords_modChrNames_for_fused_genomes.R 'Col_Chr1,Col_Chr2,Col_Chr3,Col_Chr4,Col_Chr5'

#chrName <- unlist(strsplit("Col_Chr1,Col_Chr2,Col_Chr3,Col_Chr4,Col_Chr5",
#                           split = ","))
...
# Genomic definitions
fai <- read.table("TAIR10_chr_all_renamed_fa_headers.fa.fai", header = F)
chrs <- fai$V1[1:5]
chrLens <- fai$V2[1:5]
...
# Load GFF3 of Araport11 gene coordinates
genes <- readGFF("Araport11_GFF3_genes_transposons.201606.gff")
genes$seqid <- gsub("Chr", "Col_Chr", genes$seqid)
...
write.table(mRNA_rep[,1:9],
            file = paste0(outDir, "Araport11_representative_mRNA_",
